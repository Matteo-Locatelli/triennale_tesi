...
public abstract class BitmapBuilder extends AsyncTask<Void, Void, Bitmap> {
     ...
     * @param colorshape
     */
    BitmapBuilder(ImageView testImgView, Points points, int bitmapWidth, int bitmapHeigth, int[] leftColor, int[] rightColor, int colorshape) {
        this.points = points;
        ...
        bothBit = fromRGB888toRGB565((Math.max(leftColor[0], rightColor[0])), (Math.max(leftColor[1], rightColor[1])), (Math.max(leftColor[2], rightColor[2])));
